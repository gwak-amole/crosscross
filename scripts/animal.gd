...
@onready var hitbox: Area2D = $animal_hitbox
...
func _ready() -> void:
	...
	var cb := Callable(self, "_on_animal_hitbox_area_entered")
...
func _on_animal_hitbox_area_entered(area: Area2D) -> void:
	if state != State.MOVE: return
	...
	print("[animal] contact.")
	emit_signal("contacted", self)

func _on_visible_on_screen_notifier_2d_screen_exited() -> void:
	queue_free()

func _on_animal_slowdown_area_entered(area: Area2D) -> void:
	if state != State.MOVE: return
	if area.name == "player_hitbox": return
...
func _on_animal_slowdown_area_exited(area: Area2D) -> void:
	if state != State.MOVE: return
	if area.name == "player_hitbox": return
	if area.name == "enemy_hitbox" and area.get_parent() != self:
